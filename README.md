...
- `debug_log.txt` — 新しいミノがスポーンするたび、または提案が更新されるたびに、認識した盤面・最善手をテキストで記録
...
不具合報告のたびに別の画面録画ソフトで撮り直す手間を省くため、メイン画面の「支援モード中の画面を録画する」にチェックを入れると、支援モード開始から終了まで、キャリブレーション済みの範囲（盤面+HOLD+NEXT欄）を`debug_capture.mp4`に録画し続ける（次回の支援モード開始時に上書き）。実画面をキャプチャするため、オーバーレイの提案（色ドット）も外部の画面録画ソフトで撮った場合と同様に映り込む。フレームレートはファイルサイズ・エンコード負荷を抑えるため15fps程度に間引いている（`VIDEO_RECORD_FPS`）。各フレームには`datetime.now().isoformat()`がタイムスタンプとして焼き込まれており、`debug_log.txt`・`debug_frames_history/`の記録と実時刻で直接突き合わせられる。
